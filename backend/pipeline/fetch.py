...
def _get_hashtags(description: str) -> list[str]:
    """ Extract hashtags from a given description """
    hashtags = re.findall(r'#(\w+)', description) # extract hashtags
    return hashtags


def _validate_data(data: dict) -> list:
    """ Validate the incoming video data to check for deleted videos """
    items = data["items"]
    cleaned_items = []
    for item in items:
        ...
        try:
            ...
            print(f"Dropped video: {video_id}")
    ...
    async with httpx.AsyncClient() as client:
        ...
        for result in results:
            ...
            for item in items:
                ...
                hashtags = _get_hashtags(video_data.description) # extract hashtags if present in description
